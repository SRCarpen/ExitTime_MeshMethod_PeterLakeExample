# Solve backward Fokker-Planck for Exit Time with Babak's pseudocode Dec 28 2019 email
...
# Copyright Stephen R. Carpenter 18 December 2020
...
#save(Tvec,Xvar,avec,Drift.vec,TotSig.vec,xeq,file='Step3_DDJ_Peter08-11.Rdata')
load(file='Peter1315_DDJresult.Rdata')

# Calculate D2 from TotSig.vec
D2.from.Sig = 0.5*TotSig.vec*TotSig.vec

windows()
par(mfrow=c(1,1),mar=c(4,4.5,2,2)+0.1,cex.axis=1.6,cex.lab=1.6)
xrange=range(avec)
yrange = range(c(Drift.vec,TotSig.vec),na.rm=T)
plot(avec,Drift.vec,type='l',lwd=2,col='black',xlim=xrange,ylim=yrange,
     xlab='Chlorophyll Level',
     ylab='Drift and Diffusion')
points(avec,TotSig.vec,type='l',lty=2,lwd=2,col='black')
...
Dratio = Drift.vec/(0.5*TotSig.vec*TotSig.vec)
D1.over.D2 = function(x) {   # D2 = 0.5*sigma^2(x) for standard Langevin
  yhat = approx(x=avec, y=Dratio, xout=x, method='linear',rule=2)$y
  return(yhat)
}
...
eprange = range(xvec)
xvec.ep = seq(eprange[1],eprange[2],length.out=200)
...
for(i in 1:200) {
  ...
  D1D2adj[i] = log(0.5*D2(xvec.ep[i])^2) - D1D2[i]
}
...
for(i in 1:199) {
  ...
  logdiff = log(0.5*D2(xhalf)^2)  # D2 = 0.5*sigma^2 for standard Langevin
...
plot(xvec,negPF,type='l',lwd=2,col='black',xlab='Chl Level',ylab='Potential')
plot(xvec.ep[2:200],EPF,type='l',lwd=2,col='black',xlab='Chl Level',ylab='Effective Potential')
...
x = seq(-6,xeq[2],length.out=30)  # wide range
...
# Right basin 
  ...
  x = seq(xeq[2],6,length.out=30)  # wide interval
  # solve with bvpcol or bvptwp
  trycol <- bvptwp(yini = yini, x = x, func = feval2, yend = yend, parm=plist)
...
save(Tstep,Xvar,avec,Drift.vec,TotSig.vec,xeq,xvec,drift,diff,negPF,
     xvec.ep,EPF,x,wts,ETL,ETR,xL,wtsL,xR,wtsR,
     file='Results_for_plots_Peter1315.Rdata')
