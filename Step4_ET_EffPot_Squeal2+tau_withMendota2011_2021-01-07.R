...
#
#load('Paul0811_DDtauweightsv2.Rdata')
#load('Paul1315_DDtauweightsv2.Rdata')
#Fname.out = c('Results_tauweights_plots_Paul1315.Rdata')
#
#load('Peter0811_DDtauweightsv2.Rdata')
load('Peter1315_DDtauweightsv2.Rdata')
Fname.out = c('Demo_tauweights_plots_Peter1315.Rdata')
#
windows()
par(mfrow=c(1,1),mar=c(4,4.5,2,2)+0.1,cex.axis=1.6,cex.lab=1.6)
xrange=range(avec)
yrange = range(c(D1,sigma),na.rm=T)
plot(avec,D1,type='l',lwd=2,col='black',xlim=xrange,ylim=yrange,
     xlab='Chlorophyll Level',
     ylab='Drift and Diffusion')
points(avec,sigma,type='l',lty=2,lwd=2,col='black')
grid()
...
D1fun = function(x) {
  yhat = approx(x=avec, y=D1,xout=x,method='linear',rule=2)$y
  return(yhat)
}

D2fun = function(x)  {
  yhat = approx(x=avec, y=D2,xout=x,method='linear',rule=2)$y
  return(yhat)
}
